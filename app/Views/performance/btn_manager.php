...
<div class="space-y-6">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
            ...
            <p class="text-slate-400 mt-1">Configuración operativa, conectividad, reloj, radio AAT, LF y telemetría de cada nodo.</p>
        </div>
    ...
    <section class="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-xl p-4"><p class="text-xs text-slate-400">API Connected</p><p id="btnApiConnected" class="text-2xl font-bold text-sky-400">0</p></div>
    ...
    <section class="bg-slate-900 border border-slate-800 rounded-xl p-5">
        ...
        <form id="btnForm" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div><label class="text-sm text-slate-400">Hardware</label><input id="hardwareVersion" placeholder="BTN-V0" class="mt-1 w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2"></div>
            ...
            <div><label class="text-sm text-slate-400">Servidor NTP</label><input id="ntpServer" placeholder="192.168.50.10" class="mt-1 w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2"></div>
            <div><label class="text-sm text-slate-400">Zona horaria</label><input id="timezone" value="America/El_Salvador" class="mt-1 w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2"></div>
    ...
    <section class="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
        <div class="p-5 border-b border-slate-800 flex items-center justify-between"><div><h2 class="font-bold text-xl">Nodos de Timing</h2><p class="text-sm text-slate-500 mt-1">Estado integral reportado por cada BTN.</p></div><button onclick="loadBtns()" class="px-3 py-2 bg-slate-800 rounded-lg text-sm hover:bg-slate-700">Actualizar</button></div>
        <div class="overflow-x-auto"><table class="w-full text-sm"><thead class="bg-slate-950/60 text-slate-400"><tr><th class="text-left p-3">Nodo / TP</th><th class="text-left p-3">Red / API</th><th class="text-left p-3">Reloj</th><th class="text-left p-3">AAT / LF</th><th class="text-left p-3">Queue / Salud</th><th class="text-left p-3">Última conexión</th><th class="text-left p-3">Acción</th></tr></thead><tbody id="btnTable"></tbody></table></div>
    </section>
</div>
